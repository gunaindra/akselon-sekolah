...
class Model_lihatnilaisiswa extends CI_Model {
	...
	public function getdata($paging){
	    ...
	    $this->db->select("nilai.id as id, ruang.nama as nm_ruang,siswa.nama as nm_siswa, nilai.tmnilai_siswa as  nilai, nilai.tmnilai_status as status_nilai,pj.nama as pelajaran");
        ...
        $this->db->join('tm_pelajaran as pj','pj.id=nilai.tmpelajaran_id');
  ...
  //       $this->db->from('v_siswa');
	    ...
	    if($tmpelajaran_id !=""){ $this->db->where("pj.tmpelajaran_id",$tmpelajaran_id); }
