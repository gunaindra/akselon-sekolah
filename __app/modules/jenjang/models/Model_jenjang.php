...
class Model_jenjang extends CI_Model {
	...
	public function getdata($paging){
		...
		$this->db->where("tmsekolah_id",$_SESSION['tmsekolah_id']);
         ...
         if($paging==true){
					 ...
					 $this->db->order_by("urutan",$_REQUEST['order'][0]['dir']);
			 }
	 ...
	 public function insert() {
			...
			$this->Acuan_model->log($_SESSION['nama']." Entry Jenjang Pendidikan");
	...
	public function update($id) {
			...
			$this->Acuan_model->log($_SESSION['nama']." Update Jenjang Pendidikan");	
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
